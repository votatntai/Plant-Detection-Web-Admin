 ...
 */
    getClasses(pageNumber: number = 0, pageSize: number = 10, sort: string = 'name', order: 'asc' | 'desc' | '' = 'asc', search?: string, status?: string):
        ...
        return this._httpClient.get<{ pagination: Pagination; data: Class[] }>(this.baseUrl + '/api/classes', {
            params: {
                pageSize: pageSize,
...
* Reject class
*/
    rejectClass(id: string, note: string) {
        return this.classes$.pipe(
            take(1),
            switchMap((classes) => this._httpClient.put<Class>(this.baseUrl + '/api/classes/' + id, { status: 'Rejected', note: note }).pipe(
                map((updatedClass) => {
